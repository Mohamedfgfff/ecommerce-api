...
$date = $_GET['date'] ?? null;  // REQUIRED YYYY-MM-DD
...
try {
    ...
    $sql = "
      ...
      FROM chats c
      WHERE {$whereSql}
        AND (SELECT sender_type FROM messages m WHERE m.chat_id = c.id ORDER BY m.id DESC LIMIT 1) = 'user'
    ...
    echo json_encode(['status'=>'success','pending_chats'=>$rows]);
